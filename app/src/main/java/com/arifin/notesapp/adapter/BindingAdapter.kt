...
import com.arifin.notesapp.ui.list.ListFragmentDirections
import com.google.android.material.floatingactionbutton.FloatingActionButton

class BindingAdapter {
    companion object{
        @BindingAdapter("android:navigateToAddFragement")
        @JvmStatic
        fun navigateToAddFragement(view: FloatingActionButton, navigate: Boolean){
        ...
        fun parsePriorityColor(cardView: CardView, priority: Priority){
            when(priority){
                Priority.HIGHT -> {cardView.setBackgroundColor(cardView.context.getColor(R.color.red))}
        ...
        fun parsePriorityToInt(view: Spinner, priority: Priority){
            when(priority){
                Priority.HIGHT -> {view.setSelection(0)}
